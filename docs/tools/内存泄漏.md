...
引擎虽然针对垃圾回收做了各种优化从而尽可能的确保垃圾得以回收，但是我们代码中依然要主动避免一些不利于引擎做垃圾回收操作，因为不是所有无用对象内存都可以被回收的，那当不再用到的对象内存，没有及时被回收时，我们叫它 `内存泄漏（Memory leak）`。
...
`WeakMaps` 和 `WeakSets` 是我们在 `JavaScript` 使用弱引用唯一途径，将一个对象作为键添加到 `WeakMap` 或 `WeakSet` 中并不能防止这些对象被回收。
...
重写 `obj` 以后，`{id: 1}` 依然会存在于内存中，因为 `user` 对象以及后面的 `set`/`map` 都强引用了它，`Set`/`Map`、对象、数组对象等都是强引用，所以仍然可以获取到 `{id: 1}` ，想要清除那就只能重写所有引用将其置空了。
